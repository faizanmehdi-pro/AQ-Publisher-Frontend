...
const SearchDuplicateContent = ({ onBack }) => {
    ...
    return (
        <>
            {file ? (
                <PDFViewerWrapper>
                    <BackIcon onClick={() => setFile(null)}><KeyboardBackspaceIcon /></BackIcon>
                        <FileInfo>
                          Selected File: {fileName ? <span> {fileName}</span> : null}
                        </FileInfo>

                        <Button onClick={extractTextFromPage}>Search Duplicate Content</Button>
                </PDFViewerWrapper>
            ) : (
                <FileInputButton>
                    <BackIcon onClick={onBack}><KeyboardBackspaceIcon /></BackIcon>
                    <h2>Search Duplicate Content</h2>
                    <p>This is the page for finding Duplicate Content.</p>
                    <FileInput ref={fileInputRef} type="file" onChange={handleFileChange} accept="application/pdf,text/csv" />
                    <FileButton onClick={handleFileButtonClick}>Upload CSV</FileButton>
                </FileInputButton>
            )}
        </>
    );
...
export default SearchDuplicateContent;
